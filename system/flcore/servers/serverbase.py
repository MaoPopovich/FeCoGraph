...
class Server(object):
    ...
    def set_clients(self, args, clientObj):
        for i, train_slow, send_slow in zip(range(self.num_clients), self.train_slow_clients, self.send_slow_clients):
            data = read_client_data(self.dataset, self.partition, i, train_ratio=self.train_ratio, alpha=self.dir_alpha)
            # data = read_client_data(self.dataset, self.partition, i, train_ratio=self.train_ratio)
            client = clientObj(args, 
                            id=i, 
                            train_samples=data.train_mask.sum(), 
                            test_samples=data.test_mask.sum(), 
    ...
    def evaluate(self, acc=None, loss=None):
        ...
        train_loss = (sum(stats_train[2])*1.0 / sum(stats_train[1])).cpu()
        accs = [(a / n).cpu() for a, n in zip(stats[2], stats[1])]
        
        self.rs_test_acc.append(test_acc)
        self.rs_train_loss.append(train_loss)
        self.rs_acc_std.append(np.std(accs))
